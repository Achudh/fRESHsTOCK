...
@Component({
	selector: 'page-TemperatureReading',
	templateUrl: 'TemperatureReading.component.html'
})

export class TemperatureReadingPage {
	searchQuery: string = '';
	transactions: TemperatureReading[] = [];
	...
	constructor(
		...
		this.serviceTemperatureReading.getAll().then((transactions) => {
	...
	ionViewWillEnter(): any {
		...
		let loading = this.loadingCtrl.create({
			content: 'Fetching all TemperatureReading transactions'
		});
		loading.present();
		return this.serviceTemperatureReading.getAll().then((transactions) => {
			this.currentTransactions = transactions;
			this.transactions = transactions;
			this.transactionsLoaded = true;
			loading.dismiss();
	...
	viewTransaction(transactionId){
		let modal = this.modalCtrl.create(TransactionViewPage, {
			transactionId: transactionId, transactionType: 'TemperatureReading'
		});
		modal.present();
	}

	addTransaction(){
		let modal = this.modalCtrl.create(TemperatureReadingForm, {
			transactionType: 'TemperatureReading', properties: this.properties, formType: 'Add'
		});
		modal.present();
			...
			if(data !== undefined && data.response && data.loading){
				...
				this.serviceTemperatureReading.getAll().then((transactions) => {
					...
					let alert = this.alertCtrl.create({
						...
						subTitle: 'The TemperatureReading transaction has been submitted with the ID: '.concat(response.transactionId),
						buttons: ['OK']
					});
