...
describe('GrowerForm', () => {
	...
	let ViewControllerMock = mockView();
  	...
  	beforeEach(async(() => {
    	...
    	TestBed.configureTestingModule({
			...
			imports: [
				IonicModule.forRoot(GrowerForm)
			],
			providers: [
				...
				{ provide: GrowerService, useClass:serviceGrower }
			]
    	})
  	}));
  	...
  	beforeEach(() => {
		...
		spyOn(serviceGrower.prototype,'get').and.callThrough();
    	fixture = TestBed.createComponent(GrowerForm);
    	component = fixture.componentInstance;
  	});

  	it('should create GrowerForm component', () => {
		expect(component instanceof GrowerForm).toBe(true);
	});
	...
	it('should detect if update form is opened', fakeAsync(() => {
		...
		component.participantId = 'Grower001'
		fixture.detectChanges();
	...
	it('should add an participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceGrower.prototype,'add').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		spyOn(ViewControllerMock,'dismiss');
		component.addParticipant();
		tick();

		expect(LoadingControllerMock.prototype.create).toHaveBeenCalledWith({
			content: 'Adding Grower...'
		});

		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceGrower.prototype.add).toHaveBeenCalled();
	...
	it('should throw an error for invalid add participant', fakeAsync(() => {
		...
		spyOn(serviceGrower.prototype,'add').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.addParticipant();
	...
	it('should update an participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceGrower.prototype,'update').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		spyOn(ViewControllerMock,'dismiss');
		spyOn(FormMock.prototype,'get').and.returnValue(formObjectMock);
		spyOn(FormObjectMock.prototype,'enable');
		...
		expect(LoadingControllerMock.prototype.create).toHaveBeenCalledWith({
			content: 'Updating Grower...'
		});

		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceGrower.prototype.update).toHaveBeenCalled();
		expect(ViewControllerMock.dismiss).toHaveBeenCalled();
	}));
	...
	it('should throw an error for invalid update participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'present');
		let json = JSON.stringify({error:{message:'Some error'}});
		spyOn(serviceGrower.prototype,'update').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.updateParticipant();
