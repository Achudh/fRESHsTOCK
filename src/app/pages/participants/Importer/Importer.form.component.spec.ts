...
describe('ImporterForm', () => {
	...
	let ViewControllerMock = mockView();
  	...
  	beforeEach(async(() => {
    	...
    	TestBed.configureTestingModule({
			...
			imports: [
				IonicModule.forRoot(ImporterForm)
			],
			providers: [
				...
				{ provide: ImporterService, useClass:serviceImporter }
			]
    	})
  	}));
  	...
  	beforeEach(() => {
		...
		spyOn(serviceImporter.prototype,'get').and.callThrough();
    	fixture = TestBed.createComponent(ImporterForm);
    	component = fixture.componentInstance;
  	});

  	it('should create ImporterForm component', () => {
		expect(component instanceof ImporterForm).toBe(true);
	});
	...
	it('should have correct title',() => {
		...
		let expectedTitle = 'Add Importer';
	...
	it('should detect if update form is opened', fakeAsync(() => {
		...
		component.participantId = 'Importer001'
		fixture.detectChanges();
	...
	it('should add an participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceImporter.prototype,'add').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		...
		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceImporter.prototype.add).toHaveBeenCalled();
	...
	it('should throw an error for invalid add participant', fakeAsync(() => {
		...
		spyOn(serviceImporter.prototype,'add').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.addParticipant();
	...
	it('should update an participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceImporter.prototype,'update').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		spyOn(ViewControllerMock,'dismiss');
		spyOn(FormMock.prototype,'get').and.returnValue(formObjectMock);
		spyOn(FormObjectMock.prototype,'enable');
		...
		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceImporter.prototype.update).toHaveBeenCalled();
		expect(ViewControllerMock.dismiss).toHaveBeenCalled();
	}));
	...
	it('should throw an error for invalid update participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'present');
		let json = JSON.stringify({error:{message:'Some error'}});
		spyOn(serviceImporter.prototype,'update').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.updateParticipant();
