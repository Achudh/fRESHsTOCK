...
describe('ShipperForm', () => {
	...
	let ViewControllerMock = mockView();
  	...
  	beforeEach(async(() => {
    	...
    	TestBed.configureTestingModule({
			...
			imports: [
				IonicModule.forRoot(ShipperForm)
			],
			providers: [
				...
				{ provide: ShipperService, useClass:serviceShipper }
			]
    	})
  	}));
  	...
  	beforeEach(() => {
		...
		spyOn(serviceShipper.prototype,'get').and.callThrough();
    	fixture = TestBed.createComponent(ShipperForm);
    	component = fixture.componentInstance;
  	});

  	it('should create ShipperForm component', () => {
		expect(component instanceof ShipperForm).toBe(true);
	});
	...
	it('should detect if update form is opened', fakeAsync(() => {
		...
		component.participantId = 'Shipper001'
		fixture.detectChanges();
	...
	it('should add an participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceShipper.prototype,'add').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		spyOn(ViewControllerMock,'dismiss');
		component.addParticipant();
		tick();

		expect(LoadingControllerMock.prototype.create).toHaveBeenCalledWith({
			content: 'Adding Shipper...'
		});

		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceShipper.prototype.add).toHaveBeenCalled();
	...
	it('should throw an error for invalid add participant', fakeAsync(() => {
		...
		spyOn(serviceShipper.prototype,'add').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.addParticipant();
	...
	it('should update an participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'dismiss');
		spyOn(LoadingMock.prototype,'present');
		spyOn(serviceShipper.prototype,'update').and.returnValue(new Promise((resolve,reject)=>{resolve()}));
		spyOn(ViewControllerMock,'dismiss');
		spyOn(FormMock.prototype,'get').and.returnValue(formObjectMock);
		spyOn(FormObjectMock.prototype,'enable');
		...
		expect(LoadingMock.prototype.present).toHaveBeenCalled();
		expect(serviceShipper.prototype.update).toHaveBeenCalled();
		expect(ViewControllerMock.dismiss).toHaveBeenCalled();
	}));
	...
	it('should throw an error for invalid update participant', fakeAsync(() => {
		...
		spyOn(LoadingMock.prototype,'present');
		let json = JSON.stringify({error:{message:'Some error'}});
		spyOn(serviceShipper.prototype,'update').and.returnValue(new Promise((resolve,reject) => {
			reject({_body:json})
		}));
		spyOn(ViewControllerMock,'dismiss');
		component.updateParticipant();
