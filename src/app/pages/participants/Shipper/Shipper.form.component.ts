...
export class ShipperForm {
	...
	ionViewWillEnter(){
		if(this.formType == 'Update'){
			...
			loading.present();
			return this.serviceShipper.get(this.participantId).then((participant) => {
	...
	addParticipant(){
		...
		this.serviceShipper.add(this.form.value).then(() =>{
	...
	updateParticipant(){
		...
		this.serviceShipper.update(disabledIdentifier.value, this.form.value).then(() =>{
